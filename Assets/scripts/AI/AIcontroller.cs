...
public class AIcontroller : MonoBehaviour
{
    ...
    [Range(0f, 1f)]
    public float easyAcceleration = 0.65f;

    [Range(0f, 1f)]
    public float normalAcceleration = 0.80f;

    [Range(0f, 1f)]
    public float hardAcceleration = 1.00f;

    public float easyPowerMultiplier = 1.00f;
    public float normalPowerMultiplier = 1.15f;
    public float hardPowerMultiplier = 1.30f;

    private float difficultyPowerMultiplier = 1f;
    ...
    private void AIDrive()
    {
        if (gameObject.tag == "AI")
        {
            ...
            // Difficulty controls AI speed
            switch (difficulty)
            {
                case AIDifficulty.Easy:
                    acceleration = easyAcceleration;
                    difficultyPowerMultiplier = easyPowerMultiplier;
                    break;

                case AIDifficulty.Normal:
                    acceleration = normalAcceleration;
                    difficultyPowerMultiplier = normalPowerMultiplier;
                    break;

                case AIDifficulty.Hard:
                    acceleration = hardAcceleration;
                    difficultyPowerMultiplier = hardPowerMultiplier;
                    break;
            }
    ...
    {
        Vector3 relative =
            transform.InverseTransformPoint(currentWaypoint.transform.position);

        relative /= relative.magnitude;

        horizontal =
            (relative.x / relative.magnitude) * sterrForce;
    }
    ...
    private void calculateEnginePower()
    {
        ...
        totalPower =
            3.6f *
            enginePower.Evaluate(engineRPM) *
            vertical *
    ...
    private void moveVehicle()
    {
        ...
        KPH =
            rigidbody.linearVelocity.magnitude * 3.6f;
    }
    ...
    private void animateWheels()
    {
        ...
        for (int i = 0; i < 4; i++)
        {
            ...
            wheelMesh[i].transform.position =
                wheelPosition;
    ...
    {
        CarEffects =
            GetComponent<carEffects>();

        rigidbody =
            GetComponent<Rigidbody>();

        wheelColliders =
            GameObject.Find("wheelColliders");

        wheelMeshes =
            GameObject.Find("wheelMeshes");

        wheels[0] =
            wheelColliders.transform
            .Find("0")
            .gameObject
