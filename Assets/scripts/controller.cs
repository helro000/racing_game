...
public class controller : MonoBehaviour
{
    ...
    private void FixedUpdate() {
        ...
        if (RelayRace.Instance != null && (!RelayRace.Instance.Running || hasFinished)) return;
        horizontal = IM.horizontal;
        vertical = IM.vertical;

        if (RelayRace.Instance != null) { DriveRelay(); return; }
        ...
        adjustTraction();
        if (RelayRace.Instance != null) return;
        activateNitrus();
    }

    public float RelaySteerAngle => Mathf.Lerp(30f, 13f, Mathf.Clamp01(rigidbody.linearVelocity.magnitude / 28f));

    private void DriveRelay()
    {
        float forwardSpeed = Vector3.Dot(rigidbody.linearVelocity, transform.forward);
        float speed = rigidbody.linearVelocity.magnitude;
        KPH = speed * 3.6f;
        float steer = horizontal * RelaySteerAngle;
        wheels[0].steerAngle = wheels[1].steerAngle = steer;
        bool braking = IM.handbrake || (vertical < -.1f && forwardSpeed > 1f) || (vertical > .1f && forwardSpeed < -1f);
        float throttle = braking ? 0 : vertical;
        if ((forwardSpeed > 26 && throttle > 0) || (forwardSpeed < -7 && throttle < 0)) throttle = 0;
        foreach (var wheel in wheels) {
            wheel.motorTorque = throttle * rigidbody.mass * 5f * wheel.radius / 4f;
            wheel.brakeTorque = braking ? rigidbody.mass * 8f * wheel.radius / 4f : (Mathf.Abs(vertical)<.05f ? 30 : 0);
        }
        rigidbody.linearDamping = .025f;
        rigidbody.angularDamping = 2f;
        if (System.Array.Exists(wheels, wheel => wheel.isGrounded)) {
            rigidbody.AddForce(-Vector3.up * speed * .8f, ForceMode.Acceleration);
            if(braking) {
                var planar=Vector3.ProjectOnPlane(rigidbody.linearVelocity,Vector3.up);
                rigidbody.AddForce(-planar.normalized*Mathf.Min(14f,planar.magnitude/Time.fixedDeltaTime),ForceMode.Acceleration);
            }
            // Modest lateral damping keeps keyboard steering predictable without snapping the car to the road.
            rigidbody.AddForce(-transform.right * Vector3.Dot(rigidbody.linearVelocity, transform.right) * 2f, ForceMode.Acceleration);
        }
        reverse = forwardSpeed < -.5f;
        gearNum = Mathf.Clamp((int)(KPH / 25), 0, gears.Length-1);
        engineRPM = Mathf.Lerp(1000, 6500, Mathf.Clamp01((KPH%25)/25 + Mathf.Abs(throttle)*.25f));
        playPauseSmoke = braking && KPH > 20;
        animateWheels();
    }
    ...
    private bool checkGears(){
        // Imported cars have different numbers of ratios and speed thresholds.
        if (gearChangeSpeed == null || gearChangeSpeed.Length == 0) return true;
        return KPH >= gearChangeSpeed[Mathf.Clamp(gearNum,0,gearChangeSpeed.Length-1)];
    }
    ...
    private void brakeVehicle(){
        ...
        if (IM.handbrake) { brakPower = 1800; return; }
        if (vertical < 0){
    ...
    private void steerVehicle(){
        if (RelayRace.Instance != null) {
            wheels[0].steerAngle = wheels[1].steerAngle = horizontal * 32f;
            return;
        }
    ...
    private void adjustTraction(){
        if (RelayRace.Instance != null) {
            playPauseSmoke = IM.handbrake && KPH > 15;
            return;
        }
